...
read.dba <- function(dba.file, sel=NULL, fixnames=FALSE, skip=0, col_names=TRUE,...){ # read DBA restults or any bed format file and return a genomicranges object {{{
    require(GenomicRanges)
    #require(data.table)
    #peaks <- as.data.frame(fread(dba.file))
    require(readr)
    peaks <- as.data.frame(read_tsv(dba.file, skip=skip, col_names=col_names))
